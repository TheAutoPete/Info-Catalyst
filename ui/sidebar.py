...
def render_recent_reports_sidebar(open_report, *, limit: int = 10) -> None:
...
def render_sidebar(open_report) -> bool:
    with st.sidebar:
        ...
        with st.expander("Debug options"):
            show_debug = st.checkbox("Show debug info", value=False, key="show-debug-info")
            st.caption("Shows transcript diagnostics and exception tracebacks when errors occur.")

        st.divider()
        try:
            render_source_library_section(open_report, show_debug=show_debug, result_limit=10)
        except Exception as exc:
            st.subheader("Source Library")
            st.error("Could not load Source Library.")
            if show_debug:
                st.code(f"{type(exc).__name__}: {exc}")

        st.divider()
        render_recent_reports_sidebar(open_report, limit=10)
    return show_debug
